...
- **pages/{page}/**: ルーティング単位のページコンポーネント（`index.tsx`）
- **pages/{page}/features/**: ページ専用コンポーネント（そのページでのみ使用）
- **components/ui/**: 汎用再利用可能UIコンポーネント
- **components/layout/**: レイアウト用コンポーネント
- **lib/**: ビジネスロジック・外部サービスとの連携層
- **types/domain/**: ドメインモデル（Item, Stats等のビジネス概念）
...
- `/` - 公開画面（アイテム検索・閲覧）
- `/admin` - 管理画面ダッシュボード
- `/admin/data-sync` - データ同期・メンテナンス
- `/admin/tag-and-role-management` - タグ・ロール管理
- `/dev/catalog` - コンポーネントカタログ（開発用）
...
`/dev/catalog` ページで利用可能なUIコンポーネントのカタログを確認できます：
