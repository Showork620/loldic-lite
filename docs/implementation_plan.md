...
> **⚠️ 注意（2026-07）**: このドキュメントは初期フェーズの計画書であり、データベース設計（§4）と機能設計（§6）は
> アーキテクチャ全面刷新により**古くなっています**。現行の設計は [architecture.md](./architecture.md) を参照してください。
> ディレクトリ構造ルール（§3）とデザインシステム（§5）は引き続き有効です。
